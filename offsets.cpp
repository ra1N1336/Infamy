...
void GrabOffsets()
{
	...
	offsets.m_flFlashMaxAlpha = NetVars->GetOffset(XorStr("DT_CSPlayer"), XorStr("m_flFlashMaxAlpha"));
	...
	offsets.m_bPinPulled = NetVars->GetOffset(XorStr("DT_BaseCSGrenade"), XorStr("m_bPinPulled"));
	offsets.m_bIsDefusing = NetVars->GetOffset(XorStr("DT_CSPlayer"), XorStr("m_bIsDefusing"));
	offsets.m_bHasDefuser = NetVars->GetOffset(XorStr("DT_CSPlayer"), XorStr("m_bHasDefuser"));
	offsets.m_bBombTicking = NetVars->GetOffset(XorStr("DT_PlantedC4"), XorStr("m_bBombTicking"));
	offsets.m_rgflCoordinateFrame = offsets.m_CollisionGroup - 0x30;
	...
	offsets.GlowManager = *reinterpret_cast<DWORD*>(Memory::FindPatternIDA(XorStr("client.dll"), XorStr("D1 34 A1")) + 0x3);
	offsets.GlowIndex = NetVars->GetOffset(XorStr("DT_CSPlayer"), XorStr("m_iGlowIndex"));
	offsets.Chat = *reinterpret_cast<int8_t*>(Memory::FindPatternIDA("client.dll", "E8 ? ? ? ? 8B 4F ? 85 C9 74 06 51") + 7);
	offsets.LineGoesThroughSmokeEx = Memory::FindPatternIDA("client.dll", "55 8B EC 83 EC 44 8B 15");

...
public:
	...
	int					weaponClassificationFromID(int weaponID);
	...
	bool				IsHitboxVisible(CBaseEntity * Entity, Vector Out);
	...
	{
		auto collision = static_cast<ICollideable*>(this->GetCollideable());
		Vector Max = collision->OBBMins() + this->GetOrigin();
		Vector Min = collision->OBBMaxs() + this->GetOrigin();
...
public:
	...
	int& m_bPinPulled();
	bool CanShoot();
	bool			IsShotBeingFired(CUserCmd* pUserCmd);
	int&			GetAccountID();
	int&			GetItemIDHigh();

...
{
	return VectorDistance(Global::LocalPlayer->GetOrigin(), this->GetOrigin());
}

const char* CBaseEntity::GetLastPlace()
{
	return (char*)((DWORD)this + offsets.m_szLastPlaceName);
}
...
{
	auto bombTime = *reinterpret_cast<float*>(DWORD(this) + offsets.m_flC4Blow);
	auto returnValue = bombTime - Globals->curtime;
...
void CBaseEntity::GetRenderBounds(Vector& mins, Vector& maxs)
{
	...
	GetVFunc<original>(pRenderable, 17)(pRenderable, mins, maxs);
}
...
bool CBaseEntity::IsTargetingLocal()
{
	...
	UTIL_TraceLine(src, dst, MASK_SHOT, reinterpret_cast<IHandleEntity*>(this), 0, &tr);
...
bool CBaseEntity::IsHitboxVisible(CBaseEntity* Entity, Vector Out)
{
	Ray_t ray;
	trace_t tr;
	m_visible = false;
	ray.Init(Global::LocalPlayer->GetEyePosition(), Out); // replace with config->aimbone
...
CBaseCombatWeapon* CBaseEntity::GetWeapon()
{
	...
	return  (CBaseCombatWeapon*)(ClientEntList->GetClientEntity(weaponData & 0xFFF));
}
...
int& CBaseCombatWeapon::m_bPinPulled()
{
	return *(int*)((DWORD)this + offsets.m_bPinPulled);
}
...
int CBaseEntity::weaponClassificationFromID(int weaponID) {
	int wepclass = WEPCLASS_NONE;
	if (weaponID == WEAPON_DEAGLE || weaponID == WEAPON_ELITE || weaponID == WEAPON_FIVESEVEN ||
		weaponID == WEAPON_GLOCK || weaponID == WEAPON_P250 || weaponID == WEAPON_USP_SILENCER ||
		weaponID == WEAPON_TEC9 || weaponID == WEAPON_HKP2000 || weaponID == WEAPON_CZ75A ||
		weaponID == WEAPON_REVOLVER)
		wepclass = WEPCLASS_PISTOL;

	if (weaponID == WEAPON_KNIFE || weaponID == WEAPON_KNIFE_T || weaponID == WEAPON_C4 ||
		weaponID == WEAPON_TASER || weaponID > 63)
		wepclass = WEPCLASS_INVALID;
	...
	if (weaponID == WEAPON_FLASHBANG || weaponID == WEAPON_HEGRENADE || weaponID == WEAPON_SMOKEGRENADE ||
		...
		wepclass = WEPCLASS_GRENADE;

	if (weaponID == WEAPON_AK47 || weaponID == WEAPON_AUG || weaponID == WEAPON_FAMAS ||
		weaponID == WEAPON_GALILAR || weaponID == WEAPON_M4A1 || weaponID == WEAPON_SG556 ||
		weaponID == WEAPON_M4A1_SILENCER)
		wepclass = WEPCLASS_RIFLE;

	if (weaponID == WEAPON_AWP || weaponID == WEAPON_G3SG1 || weaponID == WEAPON_SCAR20 ||
		weaponID == WEAPON_SSG08)
		wepclass = WEPCLASS_SNIPER;

	if (weaponID == WEAPON_MAC10 || weaponID == WEAPON_P90 || weaponID == WEAPON_UMP45 ||
		weaponID == WEAPON_BIZON || weaponID == WEAPON_MP7 || weaponID == WEAPON_MP9)
		wepclass = WEPCLASS_SMG;

	if (weaponID == WEAPON_XM1014 || weaponID == WEAPON_MAG7 || weaponID == WEAPON_SAWEDOFF ||
		weaponID == WEAPON_NOVA)
		wepclass = WEPCLASS_SHOTGUN;

	if (weaponID == WEAPON_M249 || weaponID == WEAPON_NEGEV)
		wepclass = WEPCLASS_MACHINEGUN;
...
int CBaseCombatWeapon::GetWeaponType()
{
	...
	//auto id = *reinterpret_cast<int*>(pWeapon + offsets.m_iItemDefinitionIndex);
	auto id = this->GetWeaponID();
	switch (id)
	{
	case WEAPON_DEAGLE:
